				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							我离XF有多远
                            <button type="button" class="btn green" style="float:right; margin-right:10px;" onClick="add_top_teach();">新增一级方法</button>
						</h3>
...
<?php	include 'footer.php';	?>
	...
	<SCRIPT type="text/javascript">
		...
		var setting = {
			async: {	//异步
				enable: true,
				url:"controller/getNodes_teach.php",
		...
		function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
			...
			//alert(parent_id);
			$.ajax({
				url:'controller/c_teach.php?act=update_parent_id_save',
		...
		function zTreeOnClick(event, treeId, treeNode) {
			id = treeNode.id;
			$.ajax({
				url:'controller/c_teach.php?act=pre_update',
				data:{'id':id},
				type:'post',
				success:function(msg){
					var obj = eval('('+msg+')');
		...
		function beforeRemove(treeId, treeNode) {	//开始删除
			...
			//alert(id);
			$.ajax({
				url:'controller/c_teach.php?act=delete',
				data:{'id':id},
				type:'post',
				success:function(msg){
					if(msg == 1){
		...
		function beforeRename(treeId, treeNode, newName) {	//开始重命名
			...
			if (newName.length == 0) {
				...
				name = nameArray[0];
				$.ajax({
					url:'controller/c_teach.php?act=update_name_save',
		...
		function addHoverDom(treeId, treeNode) {	//添加滑动DOM
			...
			if (btn) btn.bind("click", function(){
				...
				$.ajax({
					url:'controller/c_teach.php?act=add_save',
    ...
    $("#update_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_teach.php?act=update_save',
	...
	function add_top_teach(){	//新增一级方法
		$.ajax({
			url:'controller/c_teach.php?act=add_top_save',
			data:{},
			type:'post',
			success:function(msg){
				if(msg == 1){
