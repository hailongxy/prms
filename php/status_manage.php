				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							状态管理 <small></small>
							<button type="button" class="btn green" style="float:right; margin-right:10px;" data-toggle="modal" data-target="#add_modal">新增状态</button>
						</h3>
...
$sql = "SELECT COUNT(*) FROM status where userid = '".$_SESSION['userid']."'  order by id desc";
$row = mysqli_fetch_array( mysqli_query($con,$sql) );
$count = $row[0];

$last_page = (int)($count/10)+1;

$result = mysqli_query($con,"SELECT * FROM status where userid = '".$_SESSION['userid']."'  order by id desc limit ".$start_item.",10");
...
<table class="table table-bordered table-hover">
									...
									<?php	while($row = mysqli_fetch_array($result)){	?>
										<tr>
											...
											<td><a href="javascript:;" onClick="delete_status(<?php	echo $row['id'];	?>);">删除</a></td>
...
<div class="pagination pagination-centered">
									...
									<ul>
                                    	<?php	if($page > 1){	?>
										<li><a href="status_manage.php?page=1">首页</a></li>
										<li><a href="status_manage.php?page=<?php	echo	$page-1;	?>">上一页</a></li>
                                        <?php	}	?>
                                        ...
                                        <?php	if($i == $page){	?>
										<li class="active"><a href="status_manage.php?page=<?php	echo	$i;	?>"><?php	echo	$i;	?></a></li>
                                        <?php	}else{	?>
										<li><a href="status_manage.php?page=<?php	echo	$i;	?>"><?php	echo	$i;	?></a></li>
                                       	<?php	}	?> 
                                       	<?php	}	?> 
                                        
                                        
                                        <?php	if($page < $last_page){	?>
										<li><a href="status_manage.php?page=<?php	echo	$page+1;	?>">下一页</a></li>
										<li><a href="status_manage.php?page=<?php	echo	$last_page;	?>">末页</a></li>
										<?php	}	?>
									</ul>
...
<script type="application/javascript">
    $("#add_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_status_manage.php?act=add_save',
	...
	function pre_update(update_id){
		...
		//alert(id);
		$.ajax({
			url:'controller/c_status_manage.php?act=pre_update',
			data:{'id':id},
			type:'post',
			success:function(msg){
				//alert(msg);
    ...
    $(".update_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_status_manage.php?act=update_save',
	...
	function delete_status(delete_id){
		id = delete_id;
		//alert(id);
		$.ajax({
			url:'controller/c_status_manage.php?act=delete',
			data:{'id':id},
			type:'post',
			success:function(msg){
				if(msg == 1){
