...
<?php
if($_GET['type'] == 'edit'){
    $sql = mysqli_query($con,"SELECT * FROM special_diagram WHERE id ='".$_GET['id']."' and userid = '".$_SESSION['userid']."'");
    $row = mysqli_fetch_array($sql);
}
?>

<iframe id="iframe_id" width="100%" height="770px" src="drawio-dev/src/main/webapp/index.html" frameborder="0"></iframe>
...
<script>
    ...
    var diagramid = '<?php echo $_GET['diagramid'] ?? '';   ?>';
    ...
    function add(title, content) {
        $.ajax({
            url:'controller/c_special_diagram_manage.php?act=add_save',
            data:{'diagramid':diagramid,'add_title':title,'add_content':content},
            type:'post',
    ...
    function update(title, content) {
        $.ajax({
            url:'controller/c_special_diagram_manage.php?act=update_save',
