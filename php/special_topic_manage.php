...
<div id="add_modal" style="width:90%; left:25%;" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    ...
    <h3 id="myModalLabel">新增<?php echo $topic_menu[$_GET['topicid']] ?></h3>
  </div>
...
<form action="#" class="form-horizontal">
				...
				<div class="control-group">
													...
													<div class="controls" style=" margin-left:80px">
														...
														<input type="text" placeholder="" class="m-wrap medium" id="add_title" style="width:100% !important;">
...
<?php	if(in_array($_GET['topicid'],[4,236,252])){	?>
																														<option value=""></option>
														<option value="未解决">未解决</option>
															<option value="已解决">已解决</option>
															<option value="无需解决">无需解决</option>
<?php	}else if($_GET['topicid'] == 48||$_GET['topicid'] == 62||$_GET['topicid'] == 87){	?>
...
<div id="update_modal"  style="width:90%; left:25%;"  class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h3 id="myModalLabel">编辑<?php echo $topic_menu[$_GET['topicid']] ?></h3>
  </div>
  <div class="modal-body">
...
<form action="#" class="form-horizontal">
				...
				<div class="control-group">
													...
													<div class="controls" style=" margin-left:80px">
														...
														<input type="text" placeholder="" class="m-wrap medium" id="update_title" style="width:100% !important;">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							<?php echo $topic_menu[$_GET['topicid']] ?>管理 <small></small>
							<button type="button" class="btn green" style="float:right; margin-right:10px;" data-toggle="modal" data-target="#add_modal">新增<?php echo $topic_menu[$_GET['topicid']] ?></button>
						</h3>
...
$sql = "SELECT COUNT(*) FROM special_topic where userid = '".$_SESSION['userid']."'  and topicid = ".$_GET['topicid']." and (title like '%".$select_title."%' or content like '%".$select_title."%') and status like '%{$select_status}%' order by id desc";
...
$result = mysqli_query($con,"SELECT * FROM special_topic where userid = '".$_SESSION['userid']."'  and topicid = ".$_GET['topicid']." and (title like '%".$select_title."%' or content like '%".$select_title."%') and status like '%{$select_status}%' order by id desc limit ".$start_item.",10");
/*
//连接本地的 Redis 服务
$redis = new Redis();
...
while($row = mysqli_fetch_array($result)){
	//存储数据到列表中
	//$redis->lpush("tutorial-list", json_encode($row));
...
//$redis->flushall();	//删除所有
$redis->delete('tutorial-list');	//删除列表
*/
?>
...
<table class="table table-bordered table-hover">
									...
									<thead>
										...
										<tr>
											...
											<th>标题</th>
											<th>状态</th>
											<?php		if($_GET['topicid'] == 27||$_GET['topicid'] == 29){	?>

											<th>解答</th>
                                            <?php	}else if($_GET['topicid'] == 48){		?>
											<th>解决方案</th>
                                            <?php	}		?>
									...
									<?php	
									if(!empty($arList)){
										//for($i = count($arList) - 1;$i >= 0;$i--){
										foreach($arList as $row){
											//$row = json_decode($arList[$i],true);
									?>
										<tr>
											...
											<?php		if($_GET['topicid'] == 27||$_GET['topicid'] == 29||$_GET['topicid'] == 48){	?>
											...
											<td><a href="javascript:;" onClick="delete_special_topic(<?php	echo $row['id'];	?>);">删除</a></td>
...
<div class="pagination pagination-centered">
									...
									<ul>
                                    	<?php	if($page > 1){	?>
										<li><a href="special_topic_manage.php?menu_type=topic&topicid=<?php	echo	$_GET['topicid'];	?>&page=1&select_title=<?php	echo	$select_title;		?>&select_status=<?php	echo	$select_status;		?>">首页</a></li>
										<li><a href="special_topic_manage.php?menu_type=topic&topicid=<?php	echo	$_GET['topicid'];	?>&page=<?php	echo	$page-1;	?>&select_title=<?php	echo	$select_title;		?>&select_status=<?php	echo	$select_status;		?>">上一页</a></li>
                                        <?php	}	?>
                                        ...
                                        <?php	if($i == $page){	?>
										<li class="active"><a href="special_topic_manage.php?menu_type=topic&topicid=<?php	echo	$_GET['topicid'];	?>&page=<?php	echo	$i;	?>&select_title=<?php	echo	$select_title;		?>&select_status=<?php	echo	$select_status;		?>"><?php	echo	$i;	?></a></li>
                                        <?php	}else{	?>
										<li><a href="special_topic_manage.php?menu_type=topic&topicid=<?php	echo	$_GET['topicid'];	?>&page=<?php	echo	$i;	?>&select_title=<?php	echo	$select_title;		?>&select_status=<?php	echo	$select_status;		?>"><?php	echo	$i;	?></a></li>
                                       	<?php	}	?> 
                                       	<?php	}	?> 
                                        
                                        
                                        <?php	if($page < $last_page){	?>
										<li><a href="special_topic_manage.php?menu_type=topic&topicid=<?php	echo	$_GET['topicid'];	?>&page=<?php	echo	$page+1;	?>&select_title=<?php	echo	$select_title;		?>&select_status=<?php	echo	$select_status;		?>">下一页</a></li>
										<li><a href="special_topic_manage.php?menu_type=topic&topicid=<?php	echo	$_GET['topicid'];	?>&page=<?php	echo	$last_page;	?>&select_title=<?php	echo	$select_title;		?>&select_status=<?php	echo	$select_status;		?>">末页</a></li>
										<?php	}	?>
									</ul>
...
<script type="application/javascript">
    $("#add_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_special_topic_manage.php?act=add_save',
			data:{'add_title':add_title,'add_status':add_status,'add_content':add_content,'topicid':'<?php	echo	$_GET['topicid'];	?>'},
			type:'post',
	...
	function pre_update(update_id){
		...
		//alert(id);
		$.ajax({
			url:'controller/c_special_topic_manage.php?act=pre_update',
			data:{'id':id},
			type:'post',
			success:function(msg){
				//alert(msg);
    ...
    $(".update_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_special_topic_manage.php?act=update_save',
	...
	function delete_special_topic(delete_id){
		id = delete_id;
		//alert(id);
		$.ajax({
			url:'controller/c_special_topic_manage.php?act=delete',
			data:{'id':id},
			type:'post',
			success:function(msg){
				if(msg == 1){
