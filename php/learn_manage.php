...
<form action="#" class="form-horizontal">
				...
				<div class="control-group">
					...
					<?php
	                    $man_result = mysqli_query($con,"SELECT * FROM man");

					?>
                        <select class="small m-wrap" tabindex="1" id="add_title">
                            <option value=""></option>
						<?php	while($man_row = mysqli_fetch_array($man_result)){	?>
                            <option value="<?php	echo	$man_row['title'];	?>"><?php	echo	$man_row['title'];	?></option>
                        <?php	}	?>
                        </select>
				...
				<div class="control-group">
                    ...
                    <div class="controls" style=" margin-left:80px">
					<?php
	                    $status_result = mysqli_query($con,"SELECT * FROM status");

					?>
                        <select class="small m-wrap" tabindex="1" id="add_status">
                            <option value=""></option>
						<?php	while($status_row = mysqli_fetch_array($status_result)){	?>
...
<form action="#" class="form-horizontal">
				...
				<div class="control-group">
					...
					?>
                        <select class="small m-wrap" tabindex="1" id="update_title">
                            <option value=""></option>
						<?php	while($man_row = mysqli_fetch_array($man_result)){	?>
                            <option value="<?php	echo	$man_row['title'];	?>"><?php	echo	$man_row['title'];	?></option>
                        <?php	}	?>
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							学习管理 <small></small>
							<button type="button" class="btn green" style="float:right; margin-right:10px;" data-toggle="modal" data-target="#add_modal">新增学习</button>
						</h3>
...
$sql = "SELECT COUNT(*) FROM learn where userid = '".$_SESSION['userid']."'  order by id desc";
$row = mysqli_fetch_array( mysqli_query($con,$sql) );
$count = $row[0];

$last_page = (int)($count/10)+1;

$result = mysqli_query($con,"SELECT * FROM learn where userid = '".$_SESSION['userid']."'  order by id desc limit ".$start_item.",10");
...
<table class="table table-bordered table-hover">
									...
									<?php	while($row = mysqli_fetch_array($result)){	?>
										<tr>
											...
											<td><a href="javascript:;" onClick="delete_learn(<?php	echo $row['id'];	?>);">删除</a></td>
...
<div class="pagination pagination-centered">
									...
									<ul>
                                    	<?php	if($page > 1){	?>
										<li><a href="learn_manage.php?page=1">首页</a></li>
										<li><a href="learn_manage.php?page=<?php	echo	$page-1;	?>">上一页</a></li>
                                        <?php	}	?>
                                        ...
                                        <?php	if($i == $page){	?>
										<li class="active"><a href="learn_manage.php?page=<?php	echo	$i;	?>"><?php	echo	$i;	?></a></li>
                                        <?php	}else{	?>
										<li><a href="learn_manage.php?page=<?php	echo	$i;	?>"><?php	echo	$i;	?></a></li>
                                       	<?php	}	?> 
                                       	<?php	}	?> 
                                        
                                        
                                        <?php	if($page < $last_page){	?>
										<li><a href="learn_manage.php?page=<?php	echo	$page+1;	?>">下一页</a></li>
										<li><a href="learn_manage.php?page=<?php	echo	$last_page;	?>">末页</a></li>
										<?php	}	?>
									</ul>
...
<script type="application/javascript">
    $("#add_save").click(function(){//输出选中的值
		...
		//alert(add_title);
		$.ajax({
			url:'controller/c_learn_manage.php?act=add_save',
	...
	function pre_update(update_id){
		...
		//alert(id);
		$.ajax({
			url:'controller/c_learn_manage.php?act=pre_update',
			data:{'id':id},
			type:'post',
			success:function(msg){
				//alert(msg);
    ...
    $(".update_save").click(function(){//输出选中的值
		...
		var update_content = update_editor.getContent();
		$.ajax({
			url:'controller/c_learn_manage.php?act=update_save',
	...
	function delete_learn(delete_id){
		id = delete_id;
		//alert(id);
		$.ajax({
			url:'controller/c_learn_manage.php?act=delete',
			data:{'id':id},
			type:'post',
			success:function(msg){
				if(msg == 1){
