        ...
        <div class="container-fluid">
            <div class="row-fluid">
                <div class="span12">
                    <h3 class="page-title">
                        能力管理
                        <button type="button" class="btn green" style="float:right; margin-right:10px;" onClick="add_top_ability();">新增一级能力</button>
                    </h3>
                </div>
            </div>
            <div class="row-fluid">
                ...
                <div class="span6">
                    <div class="portlet box blue">
                        <div class="portlet-title">
                            <div class="caption"><i class="icon-comments"></i>能力列表</div>
                        </div>
...
var setting = {
	async: {	//异步
		enable: true,
		url:"controller/getNodes_ability.php",
...
function zTreeOnDrop(event, treeId, treeNodes, targetNode, moveType) {
	...
	$.ajax({
		url:'controller/c_ability.php?act=update_parent_id_save',
...
function zTreeOnClick(event, treeId, treeNode) {
	id = treeNode.id;
	$.ajax({
		url:'controller/c_ability.php?act=pre_update',
		data:{'id':id},
		type:'post',
		success:function(msg){
			var obj = eval('('+msg+')');
...
function beforeRemove(treeId, treeNode) {	//开始删除
	...
	$.ajax({
		url:'controller/c_ability.php?act=delete',
		data:{'id':id},
		type:'post',
		success:function(msg){
			if(msg == 1){
...
function beforeRename(treeId, treeNode, newName) {	//开始重命名
	if (newName.length == 0) {
		...
		name = nameArray[0];
		$.ajax({
			url:'controller/c_ability.php?act=update_name_save',
...
function addHoverDom(treeId, treeNode) {	//添加滑动DOM
	...
	if (btn) btn.bind("click", function(){
		...
		$.ajax({
			url:'controller/c_ability.php?act=add_save',
...
$("#update_save").click(function(){//输出选中的值
	...
	$.ajax({
		url:'controller/c_ability.php?act=update_save',
...
function add_top_ability(){	//新增一级能力
	$.ajax({
		url:'controller/c_ability.php?act=add_top_save',
		data:{},
		type:'post',
		success:function(msg){
			if(msg == 1){
