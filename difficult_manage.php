				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							难题管理 <small></small>
							<button type="button" class="btn green" style="float:right; margin-right:10px;" data-toggle="modal" data-target="#add_modal">新增难题</button>
						</h3>
...
$sql = "SELECT COUNT(*) FROM difficult where userid = '".$_SESSION['userid']."' and title like '%".$select_title."%' order by id desc";
$row = mysqli_fetch_array( mysqli_query($con,$sql) );

/**调用存储过程**/
/*
$result = mysqli_query($con,"call difficult_count(".$_SESSION['userid'].",'".$select_title."',@ret)");

$result = mysqli_query($con,"select @ret");
...
$result = mysqli_query($con,"SELECT * FROM difficult  where userid = '".$_SESSION['userid']."' and (title like '%".$select_title."%' or content like '%".$select_title."%') order by id desc limit ".$start_item.",10");
...
<table class="table table-bordered table-hover">
									...
									<?php	while($row = mysqli_fetch_array($result)){	?>
										<tr>
											...
											<td><a href="javascript:;" onClick="delete_difficult(<?php	echo $row['id'];	?>);">删除</a></td>
...
<div class="pagination pagination-centered">
									...
									<ul>
                                    	<?php	if($page > 1){	?>
										<li><a href="difficult_manage.php?page=1&select_title=<?php	echo	$select_title;		?>">首页</a></li>
										<li><a href="difficult_manage.php?page=<?php	echo	$page-1;	?>&select_title=<?php	echo	$select_title;		?>">上一页</a></li>
                                        <?php	}	?>
                                        ...
                                        <?php	if($i == $page){	?>
										<li class="active"><a href="difficult_manage.php?page=<?php	echo	$i;	?>&select_title=<?php	echo	$select_title;		?>"><?php	echo	$i;	?></a></li>
                                        <?php	}else{	?>
										<li><a href="difficult_manage.php?page=<?php	echo	$i;	?>&select_title=<?php	echo	$select_title;		?>"><?php	echo	$i;	?></a></li>
                                       	<?php	}	?> 
                                       	<?php	}	?> 
                                        
                                        
                                        <?php	if($page < $last_page){	?>
										<li><a href="difficult_manage.php?page=<?php	echo	$page+1;	?>&select_title=<?php	echo	$select_title;		?>">下一页</a></li>
										<li><a href="difficult_manage.php?page=<?php	echo	$last_page;	?>&select_title=<?php	echo	$select_title;		?>">末页</a></li>
										<?php	}	?>
									</ul>
...
<script type="application/javascript">
    $("#add_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_difficult_manage.php?act=add_save&select_title=<?php	echo	$select_title;		?>',
	...
	function pre_update(update_id){
		...
		//alert(id);
		$.ajax({
			url:'controller/c_difficult_manage.php?act=pre_update&select_title=<?php	echo	$select_title;		?>',
			data:{'id':id},
			type:'post',
			success:function(msg){
				//alert(msg);
    ...
    $(".update_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_difficult_manage.php?act=update_save&select_title=<?php	echo	$select_title;		?>',
	...
	function delete_difficult(delete_id){
		id = delete_id;
		//alert(id);
		$.ajax({
			url:'controller/c_difficult_manage.php?act=delete&select_title=<?php	echo	$select_title;		?>',
			data:{'id':id},
			type:'post',
			success:function(msg){
				if(msg == 1){
