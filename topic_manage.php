				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<h3 class="page-title">
							...
							话题管理 <small></small>
							<button type="button" class="btn green" style="float:right; margin-right:10px;" data-toggle="modal" data-target="#add_modal">新增话题</button>
						</h3>
...
$sql = "SELECT COUNT(*) FROM topic where userid = '".$_SESSION['userid']."'  order by id desc";
$row = mysqli_fetch_array( mysqli_query($con,$sql) );
$count = $row[0];

$last_page = (int)($count/10)+1;

$result = mysqli_query($con,"SELECT * FROM topic where userid = '".$_SESSION['userid']."'  order by id desc limit ".$start_item.",10");
...
<div class="pagination pagination-centered">
									...
									<ul>
                                    	<?php	if($page > 1){	?>
										<li><a href="topic_manage.php?page=1">首页</a></li>
										<li><a href="topic_manage.php?page=<?php	echo	$page-1;	?>">上一页</a></li>
                                        <?php	}	?>
                                        ...
                                        <?php	if($i == $page){	?>
										<li class="active"><a href="topic_manage.php?page=<?php	echo	$i;	?>"><?php	echo	$i;	?></a></li>
                                        <?php	}else{	?>
										<li><a href="topic_manage.php?page=<?php	echo	$i;	?>"><?php	echo	$i;	?></a></li>
                                       	<?php	}	?> 
                                       	<?php	}	?> 
                                        
                                        
                                        <?php	if($page < $last_page){	?>
										<li><a href="topic_manage.php?page=<?php	echo	$page+1;	?>">下一页</a></li>
										<li><a href="topic_manage.php?page=<?php	echo	$last_page;	?>">末页</a></li>
										<?php	}	?>
									</ul>
...
<script type="application/javascript">
    $("#add_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_topic_manage.php?act=add_save',
	...
	function pre_update(update_id){
		...
		//alert(id);
		$.ajax({
			url:'controller/c_topic_manage.php?act=pre_update',
			data:{'id':id},
			type:'post',
			success:function(msg){
				//alert(msg);
    ...
    $(".update_save").click(function(){//输出选中的值
		...
		$.ajax({
			url:'controller/c_topic_manage.php?act=update_save',
	...
	function delete_topic(delete_id){
		...
		//alert(id);
		$.ajax({
			url:'controller/c_topic_manage.php?act=delete',
			data:{'id':id},
			type:'post',
			success:function(msg){
				if(msg == 1){
